...
int main(void) {
    ...
    //リスト7.14 45度方向へのシミュレーション
    //初期状態
    //Point pos;  //座標
    //pos.x = 0.0f;
    //pos.y = 0.0f;

    //Vector speed;   //初速度(30 m/s)
    //speed.x = 1.0f;
    //speed.y = 1.0f;
    //speed = Vector_SetLength(speed, M_S(30.0f));

    //Vector acceleration;    //加速度(2 m/s^2)
    //acceleration.x = 1.0f;
    //acceleration.y = 1.0f;
    ...
    //    //移動前表示
    //    printf("%f 秒：座標: (%f, %f) 速度: (%f, %f) → ",
    //        (float)h / SECOND(1.0f), vec_acc.x, vec_acc.y, speed.x, speed.y);

    //    //移動処理
    //    Move(&vec_acc, &speed, acceleration);

    //    //移動後表示
    //    printf(" 座標: (%f, %f) 速度: (%f, %f)\n",
    //        vec_acc.x, vec_acc.y, speed.x, speed.y);
    //}
